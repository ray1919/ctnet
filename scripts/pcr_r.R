...
# go to work dir
setwd("~/ct208/r/HTqPCR/ExampleScript")
...
for (i in unique(unlist(schema5)) ) {
  if ( ! i %in% schema4[,1] && ! i %in% schema4[,4]) {
    stop(paste(i,"not mentioned.",sep = " "))
  }
}
...
for (efile in unique(schema4[,5])) {
  ...
  dir.create(subdir)
  .set(expraw,keys = efile, values = 1)
...
for (j in 1: nrow(schema4)) {
  ...
  for (i in 1:geneNum) {
    ...
    dataTbl <-  insertRow(dataTbl, c(symbol,geneid,primerid,pos,ct,tm1,tm2,
                opt.tm,NA,NA,ishousekeeping,isdoublepeak,qual))
  }
}
...
for (i in 1:nrow(dataTbl)) {
  ...
  if( !is.na(dataTbl$tm1[i]) ) {
    ...
    tms <- as.numeric(unlist(dataTbl[dataTbl$symbol == dataTbl$symbol[i],
            c('tm1','tm2')]))
    iqr <- IQR(tms,na.rm=T)
    outlimit <- c(quantile(tms,1/4,na.rm=T) - 1.5*iqr, quantile(tms,3/4,na.rm=T) + 1.5*iqr)
    istmoutlier2 <- TRUE
    for ( tmv in dataTbl[i,c('tm1','tm2')] ) {
      if (all(c(tmv > outlimit[1], tmv < outlimit[2], !is.na(tmv)) ) ) {
        opt.tm <- tmv
        istmoutlier2 <- FALSE
      }
    }
    ...
    if ( !is.na(dataTbl$primerid[i]) ) {
      ...
      if (length(tms) > 0) {
        iqr <- IQR(tms)
        outlimit <- c(quantile(tms,1/4) - 1.5*iqr, quantile(tms,3/4) + 1.5*iqr)
        istmoutlier1 <- FALSE
        if ( opt.tm < outlimit[1] || opt.tm > outlimit[2] ) {
          istmoutlier1 <- TRUE
        }
      }
    }
